...
library(qgraph)
myENG <-
  function(S, n, weights = "equal", penalty = c("fused", "group"), lambda1, lambda2, warm, penalize.diagonal=FALSE, maxiter=500, tol=1e-5, rho=1, truncate=1e-5)
  {
    p <- unique(sapply(S, ncol)) # number of variables
