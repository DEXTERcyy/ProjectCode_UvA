    ...
    if (length(neighbor_names) > 0 && n_samples_current_x >= 3) {
      ...
      if(ncol(x_predictors) > 0){
        ...
        current_nfolds <- if (n_samples_current_x < 3) { 0 } else { 
            if (n_samples_current_x <= 10) { # For very small N in the fold, use LOOCV for glmnet
                n_samples_current_x
...
pcs_cv_threshold_stabENG_lasso <- function(
  ...
  n_data_permutations = 8 # 新增参数：数据重排次数
) {
  ...
  for (p_iter in 1:n_data_permutations) {
    ...
    cv_losses_all_folds_current_perm <- matrix(Inf, nrow = length(taulist_cv), ncol = fold)
    ...
    pb_cv_inner <- utils::txtProgressBar(min = 0, max = length(taulist_cv) * fold, style = 3)
    progress_count_inner = 0

    part.list_cv_indices_target_current_perm <- cv.part_internal(n_samples_target_current_perm, fold)
    ...
    if (!is.null(current_data_matrix_other_numeric)) {
        ...
        if (n_samples_other_current_perm >= min_samples_for_cv) {
            part.list_cv_indices_other_current_perm <- cv.part_internal(n_samples_other_current_perm, fold)
        } else {
    ...
    for (k_fold in 1:fold) {
      ...
      min_train_samples = max(3, if(!is.null(stabENG_params_list$min.obs)) stabENG_params_list$min.obs else 3)
      if(length(train_indices_target) < min_train_samples || length(test_indices_target) < 1){
          ...
          progress_count_inner = progress_count_inner + length(taulist_cv); utils::setTxtProgressBar(pb_cv_inner, progress_count_inner); next
      }
