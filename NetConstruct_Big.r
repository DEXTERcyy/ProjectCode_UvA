...
for (i in timestamps){
  plot_dir <- file.path("Plots", "BigDataDaysFreedmanLane_SimNoFilter") # Changed plot dir
  ...
  if (length(plot_otus_nplus) > 0) {
    ...
    qgraph::qgraph(network_pcor[[i]]$Nplus, 
      layout = "circle",
      edge.color = ifelse(network_pcor[[i]]$Nplus > 0, "green", "red"),
  ...
  if (length(plot_otus_nminus) > 0) {
    ...
    qgraph::qgraph(network_pcor[[i]]$Nminus, 
      layout = "circle",
      edge.color = ifelse(network_pcor[[i]]$Nminus > 0, "green", "red"),
  ...
  # Use shared_otu_perm_filtered (filtered OTUs) for simulation
  ...
  # Use original data filtered to current_sim_otus for simulation
  ...
  # Use FILTERED pcor networks as true networks for simulation (network_pcor contains the filtered results)
  true_net_sim_Nplus <- if (!is.null(network_pcor[[i]]$Nplus) && ncol(network_pcor[[i]]$Nplus) > 0 && nrow(network_pcor[[i]]$Nplus) > 0) {
                            # network_pcor already contains filtered results, use it directly
                            network_pcor[[i]]$Nplus[rownames(network_pcor[[i]]$Nplus) %in% current_sim_otus, 
                                                    colnames(network_pcor[[i]]$Nplus) %in% current_sim_otus, drop = FALSE]
                        } else { matrix(0, length(current_sim_otus), length(current_sim_otus), dimnames=list(current_sim_otus, current_sim_otus)) }
  
  true_net_sim_Nminus <- if (!is.null(network_pcor[[i]]$Nminus) && ncol(network_pcor[[i]]$Nminus) > 0 && nrow(network_pcor[[i]]$Nminus) > 0) {
                             # network_pcor already contains filtered results, use it directly
                             network_pcor[[i]]$Nminus[rownames(network_pcor[[i]]$Nminus) %in% current_sim_otus, 
                                                      colnames(network_pcor[[i]]$Nminus) %in% current_sim_otus, drop = FALSE]
                         } else { matrix(0, length(current_sim_otus), length(current_sim_otus), dimnames=list(current_sim_otus, current_sim_otus)) }
  ...
  cat('Current simulation OTU count on Day', i, ':', length(current_sim_otus), '(using filtered OTUs)\n')
  cat('True network edge count for simulation - Nplus:', sum(true_net_sim_Nplus != 0)/2, 'Nminus:', sum(true_net_sim_Nminus != 0)/2, '\n')

  # Generate simulation data using filtered networks
  ...
  for (j in 1:n_sim) {
      if ( (is.matrix(Sim_list[[i]][[j]]$Nplus) && nrow(Sim_list[[i]][[j]]$Nplus) > 0 && ncol(Sim_list[[i]][[j]]$Nplus) > 0) || 
          ...
          # Use current_sim_otus as labels for stabENG
          sim_data_labels <- current_sim_otus
      ...
      # DO NOT apply any filtering to simulation results - keep them as raw stabENG output
      # Only remove diagonal elements
      ...
      # Plot sim network (using unfiltered Res_sim pcor)
      if (length(sim_data_labels) > 0) {
          ...
          if(!is.null(Res_sim[[i]][[j]]$opt.fit.pcor$Nplus) && nrow(Res_sim[[i]][[j]]$opt.fit.pcor$Nplus) > 0){
            png(filename=paste0(plot_path,"_simulation_network_",j,"_Nplus_Phylum_FilteredInput_UnfilteredOutput_vsized.png"))
            qgraph::qgraph(Res_sim[[i]][[j]]$opt.fit.pcor$Nplus, 
              layout = "circle",
              edge.color = ifelse(Res_sim[[i]][[j]]$opt.fit.pcor$Nplus > 0, "green", "red"),
              title = paste0("Sim ", j, " Network Nplus by Phylum (Day ", i, ") - Filtered Input, Unfiltered Output"),
              vsize = 2.5,
          ...
          if(!is.null(Res_sim[[i]][[j]]$opt.fit.pcor$Nminus) && nrow(Res_sim[[i]][[j]]$opt.fit.pcor$Nminus) > 0){
            png(filename=paste0(plot_path,"_simulation_network_",j,"_Nminus_Phylum_FilteredInput_UnfilteredOutput_vsized.png"))
            qgraph::qgraph(Res_sim[[i]][[j]]$opt.fit.pcor$Nminus, 
              layout = "circle",
              edge.color = ifelse(Res_sim[[i]][[j]]$opt.fit.pcor$Nminus > 0, "green", "red"),
              title = paste0("Sim ", j, " Network Nminus by Phylum (Day ", i, ") - Filtered Input, Unfiltered Output"),
              vsize = 2.5,
              groups = Phylum_groups_sim)
            dev.off()
          }
  ...
  # For confusion matrix comparison: use FILTERED pcor networks as true networks
  # This ensures we're comparing against the statistically validated network structure
  true_adj_Nplus_for_confusion <- if (!is.null(network_pcor[[i]]$Nplus) && nrow(network_pcor[[i]]$Nplus) > 0 && ncol(network_pcor[[i]]$Nplus) > 0) {
                                      (network_pcor[[i]]$Nplus != 0) * 1
                                  } else { matrix(0, length(current_sim_otus), length(current_sim_otus), dimnames=list(current_sim_otus, current_sim_otus)) }
  ...
  # Ensure proper dimensions for confusion matrix
  if(length(current_sim_otus) > 0){
      if(nrow(true_adj_Nplus_for_confusion) == 0 || ncol(true_adj_Nplus_for_confusion) == 0) {
          true_adj_Nplus_for_confusion <- matrix(0, length(current_sim_otus), length(current_sim_otus), dimnames=list(current_sim_otus, current_sim_otus))
      }
      if(nrow(true_adj_Nminus_for_confusion) == 0 || ncol(true_adj_Nminus_for_confusion) == 0) {
          true_adj_Nminus_for_confusion <- matrix(0, length(current_sim_otus), length(current_sim_otus), dimnames=list(current_sim_otus, current_sim_otus))
      }
  }

  cat('True adjacency matrices for confusion matrix (filtered) - Nplus edges:', sum(true_adj_Nplus_for_confusion)/2, 'Nminus edges:', sum(true_adj_Nminus_for_confusion)/2, '\n')

  # Confusion matrix calculation - comparing unfiltered simulation results against filtered true networks
  confusion_results <- lapply(1:n_sim, function(j_sim) {
      sim_pcor_nplus <- Res_sim[[i]][[j_sim]]$opt.fit.pcor$Nplus
      sim_pcor_nminus <- Res_sim[[i]][[j_sim]]$opt.fit.pcor$Nminus

      # Adjacency from simulation (unfiltered - keep all stabENG output)
      ...
      Nplus_metrics <- if(length(current_sim_otus) > 0 && ncol(true_adj_nplus_aligned) > 0 && ncol(adj_sim_nplus) > 0 && 
                       ...
                       } else { list(TPR=NA, FPR=NA, Precision=NA, Recall=NA, F1=NA, AUC=NA, MCC=NA, TP=NA,FP=NA,TN=NA,FN=NA) }
